...
`copy_repo_snapshot` は `repo_snapshot/` 全体の SHA-256 を計算し、ローカルの
`repo.sha256` マーカーと一致すればコピーを**スキップ**する。毎ジョブの
rmtree→copytree による SMB 帯域の浪費を避けるためで、短時間ジョブを大量に流すほど効く。
